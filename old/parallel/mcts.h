...
struct MCTS
{
  ...
  int_fast8_t simulate(Node* node);
  uint_fast8_t run(Board& b);

  Node* selectBestUCT(Node* node);
  Node* selectMostVisited(Node* node);
...
struct Node
{
  ...
  std::bitset<moves> simulatedMoves;
  Board b;
  Node* root = nullptr;
